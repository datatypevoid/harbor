...
main () {
  ...
  local _package_script=""
  config_get _package_script "script:package"

  local _DEFAULT_MOUNT_TARGET=""
  config_get _DEFAULT_MOUNT_TARGET "app-mount-target"

  local _DEFAULT_DOCKER_IMAGE=""
  config_get _DEFAULT_DOCKER_IMAGE "package-container-image"
...
package_app () {
  ...
  local _package_script=${5:?}
  local _script_target=${6:?}
  local _opts=""
  debug 1 "INFO: %s\n" "Constructing options to pass to ${_package_script}"
  _opts="$(construct_options ${_verbosity} ${_build_type} ${_mount_target})"
  debug 2 "INFO: options -- %s\n" "${_opts}"
  debug 2 "INFO: %s\n" "Copying ${_package_script} to container ${_container_name}"
  docker cp ${_package_script} ${_container_name}:${_script_target}

...
start_container () {
  ...
  local _docker_image=${2:?}
  local _mount_source=${3:?}
...
serve_app () {
  ...
  local _mount_target=${4:?}
  local _serve_script=${5:?}
  local _script_target=${6:?}
  local _opts=""
  debug 1 "INFO: %s\n" "Constructing options to pass to ${_serve_script}"
  _opts="$(construct_options ${_verbosity} ${_build_type} ${_mount_target})"
  debug 2 "INFO: options -- %s\n" "${_opts}"
  debug 2 "INFO: %s\n" "Copying ${_serve_script} to container ${_container_name}"
  docker cp ${_serve_script} ${_container_name}:${_script_target}

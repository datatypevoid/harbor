...
process_options () {
  ...
  if [[ "$#" != "0" ]]; then
    die "ERROR: invalid argument(s) '%s' passed to harbor image save.\nSee 'harbor image save --help'\n" "${@}"
  fi
...
validate () {
  local _image=$1
  if [[ -z "$_image" ]]; then
    die "ERROR: harbor image save requires at least one argument.\nSee 'harbor image save --help'\n"
  fi
}

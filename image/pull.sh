...
process_options () {
  ...
  if [[ "$#" != "0" ]]; then
    die "ERROR: invalid argument(s) '%s' passed to harbor image pull.\nSee 'harbor image pull --help'\n" "${@}"
  fi
...
validate () {
  local _name=$1
  if [[ -z "$_name" ]]; then
    die "ERROR: harbor image pull requires at least one argument.\nSee 'harbor image pull --help'\n"
  fi
}
